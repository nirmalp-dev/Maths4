...
/*
		List<KeyWord> kwl = mXparser.getKeyWords();
		String line = "\n";
		line = line + "|---|---|---|---|---|" + "\n";
		for (KeyWord kw : kwl) {
			line = line + "| " + kw.wordString + " | " + mXparser.getTokenTypeDescription(kw.wordTypeId) + " | " + kw.description + " | " + kw.syntax + " | " + kw.since + " |" + "\n";
		}
		mXparser.consolePrintln(line);
*/
		...
		|---|---|---|---|---|
		*/
		//mXparser.initPrimesCache(100000000);
		/*
		double n = 123456789;
		double[] factors = NumberTheory.primeFactors(n);
		double s = 1;
		for (double f : factors) {
			mXparser.consolePrint(f + ", ");
			s = s*f;
		}

		mXparser.consolePrintln(s);
		mXparser.consolePrint( NumberTheory.numberOfPrimeFactors(n));
		*/
		/*
		double[] tab = {1,1,1,1,1,1};
		double[][] res = NumberTheory.getDistValues(tab, true);
		for (int i = 0; i < res.length; i++) {
			mXparser.consolePrintln(i + ": v = " + res[i][0] + ", c = " + res[i][1] + ", p = " + res[i][2]);
		}
		*/
		//mXparser.consolePrintHelp("1.0");

		//Expression e = new Expression("( med(1)-1 ) + ( med(1,2)-1.5 ) + ( med(2,1)-1.5 ) + ( med(2,3,1)-2 ) + ( med(1,2,3)-2 ) + ( med(1,2,3,4)-2.5 ) + ( med(2,3,1,4)-2.5 ) + ( med(1,2,2,3)-2 )");
		//Expression e = new Expression("( med(1,2,2,3) )");
		//Expression e = new Expression("sum(n, 1, 1000000,  prod( id, 1, nfact(n), factval(n, id)^factexp(n, id) ) - n  )");
		//Expression e = new Expression("0^0");

		//mXparser.consolePrintln(e.calculate());
		//mXparser.consolePrintln(e.getComputingTime() + " s.");
		//Function f = new Function("f(x) = if( ispr(x), 0, sum(i, 1, nfact(x), factval(x, i) )) ");
		//for (int i = 1; i < 10000; i++) {
		//	System.out.println( f.calculate(i));
		//}
		//mXparser.consolePrintln( MathFunctions.root(3,125) );
		//mXparser.consolePrintln( Statistics.median(1,2,3,4));
		//mXparser.consolePrintln( mXparser.convDecimal2OthBase(-1245345.01, 10) );
		//mXparser.consolePrintln( mXparser.convDecimal2OthBase(-123, 1, 2) );
		//e.setVerboseMode();
		//mXparser.consolePrintln(e.calculate());
		/*
		...
		*/
		...
		//double a = 00000001.3223e-000010;
		//e.setVerboseMode();
		//e.calculate();
		...
		//Expression e = new Expression("sum(i, 0, 4, list(i) )", list);
		//mXparser.consolePrintln(e.getExpressionString() + " = " + e.calculate());
...
*/
		...
		*/
		...
		Expression e = new Expression("if(a=6,-b,15)", a, b);
		e.setVerboseMode();
		e.calculate();
		*/
		...
		Expression e = new Expression("fib1(10)", fib1, fib2);
		e.setVerboseMode();
		mXparser.consolePrintln("Res = " + e.calculate());
		*/
		...
		*/
		...
		//mXparser.consolePrintHelp("inver");
		/*
		long t1, t2;
		double time;
		Argument x = new Argument("x");
		...
		Expression e;
		e = new Expression("sqrt((2*x+sin(x)-x^2)/cos(x))", x);
		...
		time = (t2-t1) / 1000.0;
		mXparser.consolePrintln(time + " s.");
		*/
		//e = new Expression("5!^2"); mXparser.consolePrintln(e.calculate());
		//e = new Expression("5!-3"); mXparser.consolePrintln(e.calculate());
		//e = new Expression("5!+3"); mXparser.consolePrintln(e.calculate());
		//e = new Expression("10/5-(5!)"); mXparser.consolePrintln(e.calculate());
		//e = new Expression("10/5+(5!)"); mXparser.consolePrintln(e.calculate());
		//e = new Expression("10/5+5!"); mXparser.consolePrintln(e.calculate());
	}
}
